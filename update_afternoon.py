...
TODAY_EN = 'Sunday, April 26, 2026'
TODAY_JA = '2026年4月26日（日）'
TITLE_DATE = 'Apr 26, 2026'
...
japan_sources = [
    gnews("Japan brands yen falls as speculative as Iran war ignites sell-off Reuters"),
    gnews('Bank of Japan seen dropping hawkish signs even as it keeps rates steady Reuters'),
...
japan = [
    ...
    {
        'tag': '🇯🇵 JAPAN · FX / INTERVENTION RISK',
        'headline': 'Tokyo called the yen selloff “speculative” because the Iran-war shock pushed investors into dollars faster than Japanese authorities were willing to tolerate.',
        'body': '<strong>Why it happened:</strong> the yen weakened because global money moved toward the dollar once energy and geopolitical risk rose again. Tokyo’s response matters because officials were signaling that this was not only a fundamentals story, but also a positioning story. That causal chain raises intervention risk: when policymakers think the move is being amplified by one-way speculation, they become more willing to lean against it.',
        'sources': [japan_sources[1]],
    },
    {
        'tag': '🇯🇵 JAPAN · BOJ / POLICY',
        'headline': 'The Bank of Japan is expected to drop some hawkish language because the imported-energy shock is rising just as domestic demand still looks too fragile for a tougher stance.',
        'body': '<strong>Why it happened:</strong> Reuters’ framing is important here: the BOJ can see higher oil risk, but it also knows that tightening the message too much into a soft demand backdrop could worsen the shock. That is why the likely move is rhetorical caution. The cause is the policy trap itself, where a weaker yen feeds inflation but weak growth limits how aggressively the BOJ can respond.',
...
japan_ja = [
    ...
    ('🇯🇵 日本・円安/介入警戒', '政府が円安を「投機的」と表現した。イラン戦争でドル逃避が強まり、当局が許容しにくい速度で円売りが進んだからだ。', '<strong>なぜそうなったか：</strong>地政学リスクと原油高が重なると、資金はまずドルへ向かいやすい。そこに投機筋の片方向ポジションが重なると、当局は「ファンダメンタルズ以上の動き」と見なしやすくなる。つまり今回の発言は、単なる口先介入ではなく実弾介入の警戒感を高めるシグナルでもある。', [japan_sources[1]]),
    ('🇯🇵 日本・日銀', '日銀は据え置きでもタカ派色を弱める公算がある。輸入インフレが強まる一方、内需がまだ弱く、強い引き締めメッセージを出しにくいからだ。', '<strong>なぜ難しいか：</strong>円安は物価を押し上げるが、需要自体が十分に強いわけではない。このため日銀は、インフレ対応で引き締めたい気持ちと、景気を傷めたくない事情の両方を抱えている。因果関係としては「油高→円安→輸入インフレ」だが、「内需弱い→強くは締められない」が同時に走っている。', [japan_sources[0]]),
...
def build_body(lang='en'):
    if lang == 'en':
        ...
        sub = '🇯🇵 Tokyo held up on tech earnings, while yen-intervention risk and a softer BOJ tone showed how the oil shock is complicating Japan policy into the close · Japan also tapped shared oil reserves and saw resilience-focused corporate moves from Itochu and Denso · Globally, stocks lost confidence in a quick Iran deal, oil stayed bid, the dollar firmed, and governments from Europe to Asia kept shifting toward security-first economic policy · Health Score: 54/100'
        footer = 'CEO Afternoon Briefing · Generated by Sanbot · Sunday, April 26, 2026'
    ...
    sub = '🇯🇵 日本株はテック決算で底堅かった一方、円安けん制と日銀の慎重姿勢が、原油ショック下の政策の難しさを示した · 共同石油備蓄の活用、伊藤忠とデンソーの動きは、日本の午後テーマが「安全保障と資本効率」だったことを示した · 世界ではイラン協議停滞で原油とドルが高止まりし、各地域がより防衛的な経済運営へ傾いた · Health Score: 54/100'
    footer = 'CEO Afternoon Briefing · Generated by Sanbot · 2026年4月26日（日）'
